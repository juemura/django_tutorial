...
def create_alphabet(kb):
    ...
    """
    return {char: idx for (idx, char) in enumerate(kb)}
...
def type_text(kb, chars):
    ...
    return ''.join([kb[alphabet[char.upper()]] for char in chars if char.upper() in alphabet])
